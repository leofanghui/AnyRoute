        ...
        <div className="pt-4 border-t border-border">
          <div className="mb-3">
            ...
            <p className="text-sm text-text-muted">
              {getSettingsLabel(
                "homePinProviderQuotaToHomeDesc",
          ...
          <div className="rounded-lg border border-border bg-surface/40 divide-y divide-border/70">
            ...
            <div className="flex items-center justify-between gap-4 px-4 py-3">
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className="text-xs text-text-muted">{t("secondsUnit")}</span>
              </div>
            </div>
          </div>
        </div>
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        if (file.size > 500 * 1024) {
                          setUploadError(t("logoFileTooLarge"));
                          return;
                        }
                        const validTypes = [
                          "image/png",
                        ...
                        if (!validTypes.includes(file.type)) {
                          setUploadError(t("invalidLogoFileType"));
                          return;
                        }
                        setUploadError(null);
                        const reader = new FileReader();
                        reader.onerror = () => {
                          setUploadError(t("failedReadFile"));
                        };
                        reader.onload = (event) => {
                    ...
                    onChange={(e) => {
                      ...
                      if (file) {
                        if (file.size > 50 * 1024) {
                          setUploadError(t("faviconFileTooLarge"));
                          return;
                        }
                        const validTypes = [
                          "image/png",
                        ...
                        if (!validTypes.includes(file.type)) {
                          setUploadError(t("invalidFaviconFileType"));
                          return;
                        }
                        setUploadError(null);
                        const reader = new FileReader();
