...
export default function ModelLockoutCard() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        notify.error(error instanceof Error ? error.message : t("failedLoadModelLockout"));
      } finally {
  ...
  function validateDraft(d: ModelLockoutSettings): string | null {
    if (d.baseCooldownMs < 5000 || d.baseCooldownMs > 600000)
      return t("modelLockoutBaseCooldownRangeError");
    if (d.maxCooldownMs < 5000 || d.maxCooldownMs > 3600000)
      return t("modelLockoutMaxCooldownRangeError");
    if (d.maxCooldownMs < d.baseCooldownMs) return t("modelLockoutMaxCooldownOrderError");
    if (d.maxBackoffSteps < 0 || d.maxBackoffSteps > 20)
      return t("modelLockoutMaxBackoffStepsRangeError");
    return null;
  }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      notify.error(error instanceof Error ? error.message : t("failedSaveModelLockout"));
    } finally {
      setSaving(false);
    }
  };
        ...
        <div className="rounded-lg border border-border bg-bg-subtle p-4">
          ...
          <div className="mt-3 flex items-center gap-2">
            <input
              ...
              placeholder={t("addErrorCodePlaceholder")}
          ...
          {draft.errorCodes.length === 0 && errorCodesInput === "" && (
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              <span className="text-xs text-text-muted mr-1">{t("suggestions")}:</span>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div className="rounded-lg border border-border bg-bg-subtle p-4">
            <NumberField
              ...
              max={3600000}
              suffix="ms"
              hint={`≥ ${t("modelLockoutBaseCooldown")} — 3,600,000ms`}
