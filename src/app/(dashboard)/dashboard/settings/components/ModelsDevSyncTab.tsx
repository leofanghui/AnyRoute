  ...
  useEffect(() => {
    Promise.all([fetchStatus(), fetch("/api/settings").then((r) => (r.ok ? r.json() : null))])
      ...
      .catch((err) => {
        ...
        setFeedback({ type: "error", message: t("failedToLoad") });
      });
  }, [fetchStatus, t]);
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (result.success) {
          setFeedback({
            type: "success",
            message: t("modelsDevSyncSuccess", {
              models: result.modelCount.toLocaleString(),
              capabilities: result.capabilityCount.toLocaleString(),
            }),
          });
          fetchStatus();
        } else {
          setFeedback({ type: "error", message: result.error || t("modelsDevSyncFailed") });
        }
      } else {
        setFeedback({ type: "error", message: t("modelsDevSyncRequestFailed") });
      }
    } catch {
  ...
  const toggleEnabled = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFeedback({ type: "success", message: t("savedSuccessfully") });
      }
    } catch {
      setEnabled(!newVal);
      setFeedback({ type: "error", message: t("networkError") });
  ...
  const updateInterval = async (hours: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setFeedback({ type: "success", message: t("intervalUpdated") });
      }
    } catch {
      setIntervalHours(oldInterval);
      setDraftIntervalHours(oldInterval);
