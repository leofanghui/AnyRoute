...
export const guideSettingsSaveSchema = z
  .object({
    ...
    // #3552: CLI tool cards may post `apiKey: null` when the real key is resolved
    // server-side from keyId/default settings. Normalize null to undefined so validation
    // passes and the keyId/default path is used.
