...
function getOriginCaPoolPath() {
  return path.join(getCloudflaredRuntimeDirs().dataDir, "origin-ca-pool.pem");
}
...
function getConfiguredOriginCaPool(sourceEnv: NodeJS.ProcessEnv = process.env) {
  const candidates = [
    sourceEnv.CLOUDFLARED_ORIGIN_CA_POOL,
    sourceEnv.TUNNEL_ORIGIN_CA_POOL,
    sourceEnv.SSL_CERT_FILE,
    sourceEnv.NODE_EXTRA_CA_CERTS,
  ];

  return candidates.find((candidate) => candidate && fsSync.existsSync(candidate)) || null;
}

async function ensureCloudflaredOriginCaPool() {
  if (process.platform !== "win32") return null;

  const configured = getConfiguredOriginCaPool();
  if (configured) return configured;

  const certs = rootCertificates.filter(Boolean);
  if (!certs.length) return null;

  const caPoolPath = getOriginCaPoolPath();
  await fs.writeFile(caPoolPath, `${certs.join("\n")}\n`, "utf8");
  return caPoolPath;
}
...
export function getCloudflaredStartArgs(targetUrl: string, originCaPoolPath?: string | null) {
  const args = ["tunnel", "--url", targetUrl, "--no-autoupdate"];
  if (originCaPoolPath) {
    args.push("--origin-ca-pool", originCaPoolPath);
  }
  return args;
}
...
export async function startCloudflaredTunnel(): Promise<CloudflaredTunnelStatus> {
  ...
  startPromise = (async () => {
    ...
    const originCaPoolPath = await ensureCloudflaredOriginCaPool();
    ...
    const child = spawn(
      ...
      getCloudflaredStartArgs(targetUrl, originCaPoolPath),
      {
