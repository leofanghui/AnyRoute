...
// Regression for #3552: CLI tool cards may post `apiKey: null` when saving config and
// resolving the real key server-side from keyId/default settings.
// `apiKey: z.string().optional()` rejects `null` (Zod: "expected string, received null"), so
// the whole save 400'd. The real key is resolved server-side from keyId, so a
