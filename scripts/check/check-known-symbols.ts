...
// que silenciosamente vira no-op" em tr?s superfícies de despacho por-string/por-chave:
//
...
// uma entrada nelas exige que o par continue VIVO no registry — o oposto de
...
async function main(): Promise<void> {
  ...
  const summaryParts = [
    ...
    `${canonical.length} estrategias canonicas (${handled.size} via despacho + ${Object.keys(IMPLICIT_DEFAULT_STRATEGIES).length} default(s) implicito(s))`,
    `${livePairs.size} pares de tradutor vivos vs ${KNOWN_TRANSLATOR_PAIRS.length} congelados${newPairsNote}`,
  ];
  console.log(`[known-symbols] OK - ${summaryParts.join("; ")}`);
}
